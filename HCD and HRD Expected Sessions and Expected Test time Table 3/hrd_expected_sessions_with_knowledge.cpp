...
double computepequal2(int n,int r,vector<vector<int> >&ans)
{
	int m=ans.size();
    int sum=0;
    int p=2;
...
double computepequal3(int n,int r,vector<vector<int> >&ans)
{
    int m=ans.size();
    int sum=0;
    int p=3;
...
void less_than_three_fun(int start,int end,vector<int> &locations,bool is_even,int &count,vector<int> &tsv,int &curr_defected,int &r)
{
     	bool alter=true;
        int temp=0,vis=0;
        if(end==start)
...
double computepequal4(int n,int r,vector<vector<int> >&ans)
{
    int m=ans.size();
    int sum=0;
    int p=4;
    ...
    for(int i=0;i<n;i+=p)
    {
        ...
        if(end-start<=2)
        {
        	less_than_three_fun(start,end,locations,is_even,count,tsv,curr_defected,r);
        	fg=false;
        }
        while(fg && start <=end)
        {
...
int main() 
{
    ...
    while(r>=0){
        ...
        else
        {
            ...
            get(s,ans);
            if(p==2)
		    	avg_sessions[r]=computepequal2(n,r,ans);
		    else if(p==3)
		    	avg_sessions[r]=computepequal3(n,r,ans);
		    else if(p==4)
		    	avg_sessions[r]=computepequal4(n,r,ans);
        }
